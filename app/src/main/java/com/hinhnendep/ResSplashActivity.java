...
public class ResSplashActivity extends BaseActivity {
	private static final String TAG = ResSplashActivity.class.getSimpleName();
...
//		getActionBar().hide();
		setContentView(R.layout.res_activity_splash);

		// Picasa request to get list of albums
		String url = ResAppConst.URL_PICASA_ALBUMS
				.replace("_PICASA_USER_", ResAppController.getInstance()
		...
		JsonObjectRequest jsonObjReq = new JsonObjectRequest(Method.GET, url,
				null, new Response.Listener<JSONObject>() {
					...
					public void onResponse(JSONObject response) {
						...
						try {
							...
							for (int i = 0; i < entry.length(); i++) {
								...
								ResCategory album = new ResCategory();
								album.setId(albumId);
							...
							ResAppController.getInstance().getPrefManger()
									.storeCategories(albums);

							// String the main activity
							Intent intent = new Intent(getApplicationContext(),
									ResMainActivity.class);
					...
					public void onErrorResponse(VolleyError error) {
						...
						if (ResAppController.getInstance().getPrefManger()
								.getCategories() != null && ResAppController.getInstance().getPrefManger()
							...
							Intent i = new Intent(ResSplashActivity.this,
